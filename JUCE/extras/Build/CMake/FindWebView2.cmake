...
SUBDIRLIST(SUBDIRS "${initial_search_dir}/*Microsoft.Web.WebView2*")
if(subdirs)
    list(LENGTH subdirs num_webview2_packages)

    if(num_webview2_packages GREATER 0)
        # Loop through each found subdirectory
        foreach(search_dir IN LISTS subdirs)
            message(STATUS "Checking directory: ${search_dir}")
            set(search_dir "${search_dir}/${SUBDIRS}")
            find_path(WebView2_root_dir
                    HINTS ${search_dir}
                    PATHS ${search_dir}/build/native/include
                    NAMES WebView2.h)
            set(WebView2_root_dir "${search_dir}")
            if(WebView2_root_dir)
                set(WebView2_include_dir "${WebView2_root_dir}")
                message(STATUS "WebView2 found at: ${WebView2_include_dir}")
                break()  # Exit loop once found
            endif()
        endforeach()

        if(NOT WebView2_root_dir)
            message(FATAL_ERROR "WebView2 header not found. Ensure the SDK is properly installed.")
        endif()
    else()
        message(FATAL_ERROR "No WebView2 packages found in the local NuGet folder.")
    endif()
